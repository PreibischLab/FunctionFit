 ...
 *
 */
public class Ellipse extends AbstractFunction2D< Ellipse > implements ClosedContinousShape2D
{
	/**
	 * 
	 */
	...
	public void drawAxes( final Overlay overlay )
	{
		...
		x0 = xc + getPointXAt( axis0 );//rAxis0 * Math.cos( axis0 );
		y0 = yc + getPointYAt( axis0 );//rAxis0 * Math.sin( axis0 );
		...
		x0 = xc + getPointXAt( axis1 );//rAxis1 * Math.cos( axis1 );
		y0 = yc + getPointYAt( axis1 );//rAxis1 * Math.sin( axis1 );
		overlay.add( new Line( x0, y0, xc, yc ) );
